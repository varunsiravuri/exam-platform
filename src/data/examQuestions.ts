...
// Base question pool - we'll create variations for each exam set
const BASE_NETWORKING_QUESTIONS = [
  {
...
const BASE_WIFI_QUANT_QUESTIONS = [
  {
...
// Generate unique questions for each exam set
export function generateExamSetQuestions(examSetId: string): Question[] {
  const questions: Question[] = [];
  
  // Take 40 networking questions
  const networkingQuestions = BASE_NETWORKING_QUESTIONS.slice(0, 40).map((baseQ, index) => ({
    id: `${examSetId}_NET_${String(index + 1).padStart(3, '0')}`,
    section: 'networking' as const,
    question: baseQ.question,
    options: shuffleOptionsForSet(baseQ.options, examSetId, index),
    correctAnswer: getNewCorrectAnswer(baseQ.correctAnswer, examSetId, index),
    difficulty: baseQ.difficulty,
    examSet: examSetId
  }));

  // Take 20 Wi-Fi quantitative questions
  const wifiQuantQuestions = BASE_WIFI_QUANT_QUESTIONS.slice(0, 20).map((baseQ, index) => ({
    id: `${examSetId}_WQ_${String(index + 1).padStart(3, '0')}`,
    section: 'wifi-quant' as const,
    question: baseQ.question,
    options: shuffleOptionsForSet(baseQ.options, examSetId, index),
    correctAnswer: getNewCorrectAnswer(baseQ.correctAnswer, examSetId, index),
    difficulty: baseQ.difficulty,
  ...
  return [...networkingQuestions, ...wifiQuantQuestions];
}

function shuffleOptionsForSet(options: string[], examSetId: string, questionIndex: number): string[] {
  const shufflePattern = getShufflePattern(examSetId, questionIndex);
  return shufflePattern.map(index => options[index]);
}

function getNewCorrectAnswer(originalAnswer: number, examSetId: string, questionIndex: number): number {
  const shufflePattern = getShufflePattern(examSetId, questionIndex);
  return shufflePattern.indexOf(originalAnswer);
}

function getShufflePattern(examSetId: string, questionIndex: number): number[] {
  const patterns = {
    'SET_A': [0, 1, 2, 3], // No shuffle for Set A
    'SET_B': [1, 0, 3, 2], // Swap first two and last two
    'SET_C': [2, 3, 0, 1], // Rotate by 2
    'SET_D': [3, 2, 1, 0], // Reverse order
    'SET_E': [1, 2, 3, 0], // Rotate by 1
    'SET_F': [2, 0, 3, 1]  // Custom pattern
  };
  
  return patterns[examSetId as keyof typeof patterns] || [0, 1, 2, 3];
}

// Get questions for a specific exam set
export function getQuestionsByExamSet(examSetId: string): Question[] {
  return generateExamSetQuestions(examSetId);
}
